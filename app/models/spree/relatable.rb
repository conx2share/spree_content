module Spree
  module Relatable
    ...
    included do
      ...
      # When a model mixing in this class is destroyed, we also want to destroy all
    ...
    def has_related_content?(relation_method)
      find_relation_type(relation_method).present?
    end

    def destroy_relations
      # First we destroy relationships "from" this Class to others.
      relations.destroy_all
      # Next we destroy relationships "to" this Class.
    ...
    def find_relation_type(relation_name)
      self.class.relation_types.detect do |rt|
        rt.name.downcase.gsub(' ', '_').pluralize == relation_name.to_s.downcase
      end
